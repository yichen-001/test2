...
public class WindPowerJob {
    public static void main(String[] args) throws Exception {
        ...
        Table extractedTable = bsTableEnv.sqlQuery("select " +
                ...
                "SPLIT_INDEX(message, ',', 1) as temperature, " +
                "SPLIT_INDEX(message, ',', 2) as humidity, " +
                "SPLIT_INDEX(message, ',', 3) as rainfall, " +
                "SPLIT_INDEX(message, ',', 4) as windPower " +
                "from t_source"
        );
        bsTableEnv.createTemporaryView("t_extracted", extractedTable);
        Table filteredTable = bsTableEnv.sqlQuery(
                "select eventTime as `key`, windPower as `value` from t_extracted"
        );
